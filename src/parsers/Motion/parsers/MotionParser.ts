...
import {PacketHeaderParser} from '../../PacketHeader/PacketHeaderParser';
import {PacketMotionData} from "../types/PacketMotionData";
...
export class PacketMotionDataParser extends F1Parser {
    ...
    constructor(msg:Buffer) {
        ...
        this.nest("m_header",{type: new PacketHeaderParser})
            ...
            .array('m_suspensionPosition',{type: new Parser().floatle(''),length: 4})
            .array('m_suspensionVelocity',{type: new Parser().floatle(''),length: 4})
            .array('m_suspensionAcceleration',{type: new Parser().floatle(''),length: 4})
            .array('m_wheelSpeed',{type: new Parser().floatle(''),length: 4})
            .array('m_wheelSlip',{type: new Parser().floatle(''),length: 4})
